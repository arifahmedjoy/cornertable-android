    ...
    public class VideoAudioComingCallActivity : AppCompatActivity, ValueAnimator.IAnimatorUpdateListener, ISensorEventListener, MaterialDialog.ISingleButtonCallback, MaterialDialog.IListCallback, MaterialDialog.IInputCallback
    {
        ...
        private void RejectCallButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    case "Twilio_video_call":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallAsync(UserDetails.UserId, CallId, TypeCall.Video) });
                        break;
                    case "Twilio_audio_call":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await RequestsAsync.Call.DeclineCallAsync(UserDetails.UserId, CallId, TypeCall.Audio) });
                        break;
                    case "Agora_video_call_recieve":
                    case "Agora_audio_call_recieve":
                        PollyController.RunRetryPolicyFunction(new List<Func<Task>> { async () => await ApiRequest.Send_Agora_Call_Action_Async("decline", CallId) });
                        break;
                }
