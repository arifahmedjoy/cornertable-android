    ...
    public class LastChatsAdapter : RecyclerView.Adapter, ListPreloader.IPreloadModelProvider
    {
        ...
        public override RecyclerView.ViewHolder OnCreateViewHolder(ViewGroup parent, int viewType)
        {
            try
            {
                switch (viewType)
                {
                    case (int)Classes.ItemType.LastChatOldV:
                        ...
                        case Classes.ItemType.LastChatOldV:
                            {
                                if (viewHolder is LastChatsAdapterViewHolder holder)
                                {
                                    ...
                                    {
                                        case "WithoutBlobMute" when item.LastMessagesUser.IsMute:
                                            holder.IconMute.Visibility = ViewStates.Visible;
                                            Glide.With(ActivityContext).Load(Resource.Drawable.icon_mute_vector).Into(holder.IconMute);
                                            break;
                                        case "WithoutBlobMute":
                                            holder.IconMute.Visibility = ViewStates.Gone;
                                            break;
                                        case "WithoutBlobPin" when item.LastMessagesUser.IsPin && TypePage != "Archived":
                                            holder.IconPin.Visibility = ViewStates.Visible;
                                            Glide.With(ActivityContext).Load(Resource.Drawable.icon_pin_vector).Into(holder.IconPin);
                                            break;
                                        case "WithoutBlobPin":
                                            holder.IconPin.Visibility = ViewStates.Gone;
                                            break;
                                        case "WithoutBlobLastSeen":
                                            holder.TxtTimestamp.Text = item.LastMessagesUser.LastseenTimeText;
                                            if (item.LastMessagesUser.Lastseen == "on" && UserDetails.OnlineUsers)
                                            {
                                                ...
                                                {
                                                    var data = ListOnline.Contains(item.LastMessagesUser.Name);
                                                    if (data == false && !item.LastMessagesUser.IsMute)
                                                    {
                                                        ListOnline.Add(item.LastMessagesUser.Name);

                                                        Toast toast = Toast.MakeText(ActivityContext, item.LastMessagesUser.Name + " " + ActivityContext.GetText(Resource.String.Lbl_Online), ToastLength.Short);
                                        ...
                                        case "WithoutBlobText":
                                            InitializeLastChatOldV(holder, item.LastMessagesUser);
                                            holder.TxtLastMessages.Text = item.LastMessagesUser.LastMessage.Text;
                                            switch (string.IsNullOrEmpty(item.LastMessagesUser.LastMessage.Media))
                                            {
                                                //If message contains Media files 
                                                case false when item.LastMessagesUser.LastMessage.Media.Contains("image"):
                                                    ...
                                                    break;
                                                case false when item.LastMessagesUser.LastMessage.Media.Contains("video"):
                                                    holder.LastMessagesIcon.Visibility = ViewStates.Visible;
                                                    Glide.With(ActivityContext).Load(Resource.Drawable.ic_video_player_vector).Into(holder.LastMessagesIcon);
                                                    break;
                                                case false when item.LastMessagesUser.LastMessage.Media.Contains("sticker"):
                                                    holder.LastMessagesIcon.Visibility = ViewStates.Visible;
                                                    Glide.With(ActivityContext).Load(Resource.Drawable.ic_sticker_vector).Into(holder.LastMessagesIcon);
                                                    break;
                                                case false when item.LastMessagesUser.LastMessage.Media.Contains("sounds"):
                                                    holder.LastMessagesIcon.Visibility = ViewStates.Visible;
                                                    Glide.With(ActivityContext).Load(Resource.Drawable.ic_radios_vector).Into(holder.LastMessagesIcon);
                                                    break;
                                                case false when item.LastMessagesUser.LastMessage.Media.Contains("file"):
                                                    holder.LastMessagesIcon.Visibility = ViewStates.Visible;
                                                    Glide.With(ActivityContext).Load(Resource.Drawable.ic_document_vector).Into(holder.LastMessagesIcon);
                                                    break;
                                                default:
                                                    {
                                                        if (!string.IsNullOrEmpty(item.LastMessagesUser.LastMessage.Stickers) && item.LastMessagesUser.LastMessage.Stickers.Contains(".gif"))
                                                        {
                                                            holder.LastMessagesIcon.Visibility = ViewStates.Visible;
                                                            Glide.With(ActivityContext).Load(Resource.Drawable.ic_gif_vector).Into(holder.LastMessagesIcon);
                                                        }
                                                        else if (!string.IsNullOrEmpty(item.LastMessagesUser.LastMessage.Lat) && !string.IsNullOrEmpty(item.LastMessagesUser.LastMessage.Lng) && item.LastMessagesUser.LastMessage.Lat != "0" && item.LastMessagesUser.LastMessage.Lng != "0")
                                                        {
                                                            holder.LastMessagesIcon.Visibility = ViewStates.Visible;
        ...
        private void InitializeLastChatOldV(LastChatsAdapterViewHolder holder, GetUsersListObject.User item)
        {
            try
            {
                ...
                //last seen time  
                holder.TxtTimestamp.Text = item.LastseenTimeText;

                //Online Or offline
                if (item.Lastseen == "on" && UserDetails.OnlineUsers)
                {
                    holder.TxtTimestamp.Text = ActivityContext.GetText(Resource.String.Lbl_Online);
                    holder.ImageLastseen.SetImageResource(Resource.Drawable.Green_Online);
        ...
        public override int GetItemViewType(int position)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    {
                        Classes.ItemType.LastChatOldV => (int)Classes.ItemType.LastChatOldV,
            ...
            try
            {
                ...
                else
                {
                    switch (item.Type)
                    {
                        case Classes.ItemType.LastChatOldV:
                            {
                                var image = !string.IsNullOrEmpty(item?.LastMessagesUser?.OldAvatar) ? item?.LastMessagesUser?.OldAvatar : item?.LastMessagesUser?.Avatar ?? item?.LastMessagesUser?.Cover;
