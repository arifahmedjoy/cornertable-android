    ...
    public class ChatColorsFragment : AndroidX.Fragment.App.Fragment
    {
        ...
        private void SetTheme(Activity activity, string color)
        {
            try
            {
                ...
                {
                    var dataUser = MsgTabbedMainActivity.GetInstance()?.LastChatTab?.MAdapter?.LastChatsList?.FirstOrDefault(a => a.LastMessagesUser?.UserId == UserId);
                    if (dataUser != null)
                    {
                        dataUser.LastMessagesUser.ChatColor = color;
                    }
                }
