...
namespace WoWonder.Helpers.Utils
{
    public static class WoWonderTools  
    {
        ...
        public static async Task<bool> CheckMimeTypesWithServer(string path)
        {
            try
            {
        ...
        public static GetUsersListObject.User FilterDataLastChatOldV(GetUsersListObject.User item)
        {
            try
            {
                if (item != null)
                {
                    ...
                    item.IsMute = CheckMute(item.UserId, "user", item.Mute);
                    item.IsPin = CheckPin(item.UserId, "user", item.Mute);
                    item.IsArchive = CheckArchive(item.UserId, "user", item.Mute).Item2;
        ...
        public static MessageDataExtra MessageFilter(string id, MessageData item, MessageModelType modelType, bool showStar = false)
        {
            try
            {
                ...
                if (item.Reply?.ReplyClass != null && !string.IsNullOrEmpty(item.ReplyId))
                {
